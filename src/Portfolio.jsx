...
function FigureArt({ kind }) {
  if (kind === 'accel') return (
    <svg className="case-figure-svg" viewBox="0 0 920 440" role="img" aria-label="AccelClosure prompt to GDSII flow diagram">
      ...
      <rect x="28" y="311" width="864" height="72" rx="8" className="figure-band" /><text className="figure-band-label" x="52" y="341">CLOSED 4 × 4 · 8 × 8 · 16 × 16 REFERENCES</text><text className="figure-band-value" x="52" y="365">210.57 / 212.00 / 208.18 MHz post-route Fmax · zero setup / hold</text><text className="figure-band-label" x="745" y="341">SKY130HD</text><text className="figure-band-value" x="745" y="365">WS · INT8</text>
    </svg>
  )
  ...
  if (kind === 'attention') return (
    <svg className="case-figure-svg" viewBox="0 0 920 440" role="img" aria-label="INT4 transformer attention dataflow diagram">
      ...
      <rect x="36" y="314" width="848" height="68" rx="8" className="figure-band" /><text className="figure-band-label" x="58" y="341">CADENCE / GPDK045 SNAPSHOT</text><text className="figure-band-value" x="58" y="365">333.33 MHz · 286,546 cells · 1,201-cycle power run · 3.410 GMAC/s</text>
    </svg>
  )
...
function ProjectPage({ project }) {
  ...
  const hasImages = Boolean(project.figureImages?.length)
  useEffect(() => {
  ...
  return (
    <div className="portfolio case-study-page" id="portfolio"><Navigation /><main>
      ...
      <section className="case-figures shell"><div className="case-section-heading"><div><SectionLabel number="01">Figures & evidence</SectionLabel><h2>From architecture<br /><em>to physical proof.</em></h2></div><p>Each figure is scoped to the implementation record for this project. The visual explains the path; the metric keeps it honest.</p></div><div className="case-figure-grid"><figure className="case-figure case-figure-wide"><div className="case-figure-topline"><span>FIG. {project.index}A · SYSTEM VIEW</span><span>{project.visual === 'physical' ? 'PHYSICAL DESIGN' : 'ARCHITECTURE'}</span></div><FigureArt kind={project.visual} /><figcaption>{project.visual === 'accel' ? 'AccelClosure closes the loop from a natural-language request to a reproducible GDSII artifact.' : `A code-native schematic of the ${project.title.toLowerCase()} data path and its measured checkpoints.`}</figcaption></figure>{hasImages && project.figureImages.map((figure, index) => <figure className="case-figure case-figure-image" key={figure.src}><div className="case-figure-topline"><span>FIG. {project.index}{String.fromCharCode(66 + index)} · LAYOUT RECORD</span><span>{figure.type || 'GDSII / KLAYOUT'}</span></div><img src={figure.src} alt={figure.alt} /><figcaption>{figure.caption}</figcaption></figure>)}<figure className="case-figure case-figure-metrics"><div className="case-figure-topline"><span>FIG. {project.index}{hasImages ? String.fromCharCode(66 + project.figureImages.length) : 'B'} · IMPLEMENTATION RECORD</span><span>REPORTED CONFIGURATION</span></div><div className="case-stat-grid">{project.secondary.map(([value, label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div><div className="case-scope"><span className="note-label">Scope note</span><p>{project.note}</p></div></figure></div>{project.workload && <div className="case-workload"><div><SectionLabel number="01b">Real workload validation</SectionLabel><h3>{project.workload.title}</h3><p>{project.workload.detail}</p></div><div className="workload-table"><div className="workload-row workload-head"><span>Model</span><span>Selected GEMM</span><span>Cosine similarity</span></div>{project.workload.rows.map(([model, shape, similarity]) => <div className="workload-row" key={model}><strong>{model}</strong><span>{shape}</span><span>{similarity}</span></div>)}<p className="workload-footnote">{project.workload.footnote}</p></div></div>}{project.resultTables && <div className="case-results">{project.resultTables.map((table) => <article key={table.title}><SectionLabel number="01c">{table.title}</SectionLabel><div className="results-table"><div className="results-row results-head">{table.columns.map((column) => <span key={column}>{column}</span>)}</div>{table.rows.map((row) => <div className="results-row" key={row[0]}>{row.map((cell, index) => index === 0 ? <strong key={cell}>{cell}</strong> : <span key={cell}>{cell}</span>)}</div>)}</div></article>)}</div>}{project.physicalRecord && <div className="case-physical"><div><SectionLabel number="01d">Physical record</SectionLabel><h3><PhysicalRecordHeading project={project} /></h3></div><div className="case-physical-grid">{project.physicalRecord.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></div>}</section>
...
function PhysicalRecordHeading({ project }) {
  if (project.id === 'gemmini') return <>Proposed 16×16<br /><em>MOSA implementation.</em></>
  if (project.id === 'transformer') return <>Cadence synthesis<br /><em>record.</em></>
  if (project.id === 'cnn-tapeout') return <>Post-route<br /><em>closure record.</em></>
  if (project.id === 'fpga-cgm') return <>FPGA implementation<br /><em>record.</em></>
  if (project.id === 'cgm-asic') return <>SKY130 physical<br /><em>record.</em></>
  return <>Chip-level<br /><em>physical record.</em></>
}
...
function ProjectCardArt({ project }) {
  if (!project.figureImages?.length) return <ProjectGlyph kind={project.visual} />
  return (
    <div className="work-card-image-art">
      <img src={project.figureImages[0].src} alt={project.figureImages[0].alt} />
      <div className="work-card-image-strip">{project.figureImages.map((figure) => <span key={figure.src}><img src={figure.src} alt="" /></span>)}</div>
      <span className="work-card-image-label">{project.figureImages[0].label}</span>
    </div>
  )
}
...
function ProjectCard({ project, lead }) {
  return (
    <article className={lead ? 'work-card work-card-lead' : 'work-card'}>
      <div className="work-card-art"><span className="art-index">{project.index} / {project.year}</span><ProjectCardArt project={project} /><span className="art-caption">{project.kind}</span></div>
...
function WorkSection() {
  ...
  return (
    <section className="section shell work-section" id="work">
      <div className="section-heading"><div><SectionLabel number="01">Selected systems</SectionLabel><h2>Newest work first.<br /><em>Evidence beside the idea.</em></h2></div><p>Each case study keeps the scope visible: architecture, implementation flow, and the number that actually belongs to that configuration.</p></div>
